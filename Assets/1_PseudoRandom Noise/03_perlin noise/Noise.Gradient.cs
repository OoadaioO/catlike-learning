...
public static partial class Noise {
    ...
    public struct Value : IGradient {
        public float4 Evaluate(SmallXXHash4 hash, float4 x) => hash.Floats01A * 2f - 1;

        public float4 Evaluate(SmallXXHash4 hash, float4 x, float4 y) => hash.Floats01A * 2f - 1;

        public float4 Evaluate(SmallXXHash4 hash, float4 x, float4 y, float4 z) => hash.Floats01A * 2f - 1;
    }
    ...
    public struct Perlin : IGradient {
        ...
        public float4 Evaluate(SmallXXHash4 hash, float4 x, float4 y, float4 z) {
            ...
            gx += select(-offset, offset, gx < 0);
            gy += select(-offset, offset, gy < 0);
