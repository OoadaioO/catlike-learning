...
public class BallVisualization : MonoBehaviour {
    ...
    [Min(0f)]
    [SerializeField] private float minSpinSpeed = 20f, maxSpinSpeed = 60f;

    PrefabInstancePool<BallVisualization> pool;


    private Vector3 rotationAxis;
    private float radius, rotationSpeed, rotationAngle;

    public BallVisualization Spawn() {
        BallVisualization instance = pool.GetInstance(this);
        instance.pool = pool;
        instance.radius = -1f;
        instance.rotationAxis = Random.insideUnitSphere;
