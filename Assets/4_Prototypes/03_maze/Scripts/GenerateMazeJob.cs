...
public struct GenerateMazeJob : IJob {
    ...
    public void Execute() {
        ...
        NativeArray<(int, MazeFlags, MazeFlags)> scratchpad =
            new(4, Allocator.Temp, NativeArrayOptions.UninitializedMemory);

        NativeArray<int> activeIndices = new NativeArray<int>(
            maze.Length,
            Allocator.Temp,
        ...
        while (firstActiveIndex <= lastActiveIndex) {
            ...
            int avaliablePassageCount = FindAvailiablePassages(index, scratchpad);
            if (avaliablePassageCount <= 1) {
                if (pickLast) {
                    lastActiveIndex -= 1;
                } else {
            ...
            if (avaliablePassageCount > 0) {
                (int, MazeFlags, MazeFlags) passage = scratchpad[random.NextInt(0, avaliablePassageCount)];
    ...
    int FindAvailiablePassages(int index, NativeArray<(int, MazeFlags, MazeFlags)> scratchpad) {
